...
## Status: M13.5 — Bin master parity (part of M13, in progress)

M13 (Section 10.2: full web read/write parity, plus the SQL console/dashboards and WhatsApp inbound inbox) is the largest milestone in the build order, and is being built as a sequence of chunks rather than one pass — M13.1 (Staff & roles), M13.2 (manual-override infrastructure), M13.3 (Inventory), M13.4 (Product master), and M13.5 (Bin master) are done and verified; the rest of M13 is tracked but not yet built (see "Build order" below).
