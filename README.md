...
Milestones M0–M16 as agreed. Each one gets built, tested by the owner, then the next starts — no skipping ahead, no scope creep beyond what's specified. Current milestone: **M5**, done pending sign-off — "the single most valuable data a pharmacy throws away" now has a home: a customer request book with three intake paths, purchase orders that merge low stock with open requests, a GRN-triggered callback loop with stock reservation, and a daily review alarm that only ever fires when there's genuinely something to review. Verified end to end in the browser: log a request → receive stock → callback flips it to "received" → reserve & notify → bill the customer from the reservation → request closes out fulfilled. Next: **M6** — cycle counts, expiry audit, statutory reports.
