...
Milestones M0–M16 as agreed. Each one gets built, tested by the owner, then the next starts — no skipping ahead, no scope creep beyond what's specified. Current milestone: **M6**, done pending sign-off — cycle counts (blind, dual-entry, variance auto-escalation to Manager/Owner), an expiry audit dashboard with one-tap move-to-quarantine, a damage/write-off log with photo evidence and Owner approval above a value threshold, and the Section 10A statutory reports: sales/purchase/non-GST/credit-note registers, a GSTR-1 and GSTR-3B working, HSN summary, batch traceability for recalls, and location-wise inventory — every report exportable as CSV and clearly labelled as a working file for the owner's accountant, not a filing. Verified end to end: a real blind count with a deliberate variance escalated correctly and adjusted stock only on approval; a write-off above the threshold sat untouched until approved; the negative-stock exception report caught a genuine anomaly left over from earlier testing. Next: **M7** — prescriber master, schemes/effective-cost, credit customers, reorder levels, vendor comparison.
